...
export default function HeroPlantView({ plant, onPlantSelect }: HeroPlantViewProps) {
  ...
  const handlePlantSelect = (selectedPlant: Plant) => {
    const currentIndex = plantsData.filter(p => !['rubber-plant', 'fiddle-fig', 'zz-plant', 'aloe-vera'].includes(p.id)).findIndex(p => p.id === plant.id);
    const newIndex = plantsData.filter(p => !['rubber-plant', 'fiddle-fig', 'zz-plant', 'aloe-vera'].includes(p.id)).findIndex(p => p.id === selectedPlant.id);
    
    setDirection(newIndex > currentIndex ? 1 : -1); // 1 for down (current goes down), -1 for up (current goes up)
  ...
  return (
    <motion.section 
      className="min-h-screen flex items-center px-6 sm:px-8 md:px-12 pt-24 relative overflow-hidden"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      <div className="max-w-7xl mx-auto w-full px-4 relative z-10">
        ...
        <div className="mt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {plantsData.filter(plantItem => !['rubber-plant', 'fiddle-fig', 'zz-plant', 'aloe-vera'].includes(plantItem.id)).slice(0, 4).map((plantItem) => (
              <motion.div
                key={plantItem.id}
                className={`
